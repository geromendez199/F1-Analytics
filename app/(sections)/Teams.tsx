...
function TeamCard({ team, driverMap, liveryAlt, pointsLabel, driversLabel }: TeamCardProps) {
  return (
    <article className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg">
...
export default function Teams({ locale }: { locale: Locale }) {
  ...
  return (
    <section id="escuderias" aria-labelledby="teams-title" className="container mx-auto px-6">
      <header className="mb-8 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 id="teams-title" className="text-3xl font-semibold">{dictionary.teams.title}</h2>
          <p className="text-sm text-slate-400">{dictionary.teams.subtitle}</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {teams.map((team) => (
          <TeamCard
            ...
            team={team}
            driverMap={driverMap}
            liveryAlt={dictionary.teams.liveryAlt.replace("{team}", team.name)}
            pointsLabel={dictionary.teams.points}
            driversLabel={dictionary.teams.drivers}
          />
        ))}
      </div>
    </section>
